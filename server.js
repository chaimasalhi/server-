...
if (!CONFIG.hmacKey) {
  console.error('[FATAL] MQTT_HMAC_KEY manquant');
  process.exit(1);
}
if (!CONFIG.apiKey) {
  console.error('[FATAL] API_KEY manquant');
  process.exit(1);
}
if (!CONFIG.mqttHost) {
  console.error('[FATAL] MQTT_HOST manquant');
  process.exit(1);
}
...
async function sendPushNotification(fcmToken, title, body, data = {}) {
  ...
  try {
    ...
    log('INFO', `📱 Push envoyé à ${fcmToken.substring(0, 10)}...`);
  } catch (error) {
    log('ERROR', `Erreur envoi push: ${error.message}`);
  }
}
...
function verifyHmac(payload) {
  ...
  const sorted = Object.keys(dataWithoutHmac)
    .sort()
    .reduce((acc, k) => {
  ...
  try {
    const valid = crypto.timingSafeEqual(Buffer.from(receivedHmac, 'hex'), Buffer.from(expectedHmac, 'hex'));
...
// ============================================================
//  SAVE ALERT (modifié pour envoyer les notifications push)
// ============================================================
...
function initAccessLog() {
  if (fs.existsSync(CONFIG.logFile)) log('INFO', 'Ancien access_log.json détecté — nouvelles entrées dans MongoDB uniquement');
}
...
function buildTempReason(temperature, level) {
  switch (level) {
    case 'WARNING':
      return `Température ${temperature.toFixed(1)}°C ≥ ${CONFIG.warningTemp}°C — ASHRAE A1 dépassée`;
    case 'DANGER':
      return `Température ${temperature.toFixed(1)}°C ≥ ${CONFIG.dangerTemp}°C — limite critique ASHRAE A1`;
    default:
      return `Température ${temperature.toFixed(1)}°C — zone normale ASHRAE A1`;
  }
}
...
async function predictTemperature(temperature) {
  ...
  if (tempBuffer.length < 6) {
    return {
      ...
      temperaturePrediteA20: null,
      minutesAvantWarning: null,
      minutesAvantDanger: null,
      alerte: 'NORMAL',
      confidence: 0,
  ...
  for (let step = 1; step <= MAX_STEPS; step++) {
    ...
    if (!minutesAvantDanger && predicted >= CONFIG.salleMax) {
      minutesAvantDanger = minutes;
      break;
    }
  }
  ...
  const result = {
    temperatureActuelle: temperature,
    temperaturePrediteA20: predictedAt20min,
    minutesAvantWarning,
    minutesAvantDanger,
    dansMinutes: CONFIG.predictAhead,
    confidence,
    alerte,
    message,
    timestamp: new Date().toLocaleString(),
...
async function handleHumidity(payload) {
  ...
  if (humidity > CONFIG.humidityDanger) {
    targetLevel = 'DANGER';
    riskType = 'condensation';
  } else if (humidity < CONFIG.humidityDangerLow) {
    targetLevel = 'DANGER';
    riskType = 'esd_severe';
  } else if (humidity > CONFIG.humidityMax) {
    targetLevel = 'WARNING';
    riskType = 'warning_haut';
  } else if (humidity < CONFIG.humidityMin) {
    targetLevel = 'WARNING';
    riskType = 'warning_bas';
  } else if (trend > 8) {
    targetLevel = 'WARNING';
    riskType = 'tendance';
  }
  ...
  if (confirmed && wasNormal && riskType) {
    ...
    let seuil;
    if (riskType === 'condensation') seuil = CONFIG.humidityDanger;
    else if (riskType === 'esd_severe') seuil = CONFIG.humidityDangerLow;
    else if (riskType === 'warning_haut') seuil = CONFIG.humidityMax;
    else if (riskType === 'warning_bas') seuil = CONFIG.humidityMin;
    else seuil = CONFIG.humidityMax;
  ...
  liveData.humidity = {
    ...
    debounce: humidityState.debounceCount,
    risk_type: riskType,
  };
  liveData.lastUpdate = new Date().toLocaleString();
  wsBroadcast({ type: 'live_data', data: liveData });
...
function buildHumidityAlertMessage(humidity, riskType, trend) {
  switch (riskType) {
    case 'condensation':
      return `🚨 DANGER humidité : ${humidity.toFixed(1)}%HR > ${CONFIG.humidityDanger}%HR — Risque condensation (TIA-942-B §6.5)`;
    case 'esd_severe':
      return `🚨 DANGER humidité basse : ${humidity.toFixed(1)}%HR < ${CONFIG.humidityDangerLow}%HR — Risque ESD sévère`;
    case 'warning_haut':
      return `⚠️ WARNING humidité élevée : ${humidity.toFixed(1)}%HR > ${CONFIG.humidityMax}%HR`;
    case 'warning_bas':
      return `⚠️ WARNING humidité basse : ${humidity.toFixed(1)}%HR < ${CONFIG.humidityMin}%HR`;
    case 'tendance':
      return `⚠️ WARNING tendance humidité : +${trend.toFixed(1)}% sur ${CONFIG.humidityWindow} lectures`;
    default:
      return `⚠️ WARNING humidité : ${humidity.toFixed(1)}%HR hors zone optimale`;
  }
}
...
function buildGasAlertMessage(level, adcEma, adcRaw, risingRate) {
  ...
  if (level === 'DANGER') return `🚨 DANGER gaz : EMA=${adcEma} ADC (${pctSeuil}% du seuil critique) — IEC 60079 / EN 54-5`;
  if (risingRate >= CONFIG.gasRisingRate) return `⚠️ WARNING gaz — montée rapide : +${risingRate} ADC/lecture, EMA=${adcEma}`;
...
function connectMQTT() {
  ...
  client.on('message', async (topic, rawMsg) => {
    let payload;
    try {
      payload = JSON.parse(rawMsg.toString());
    } catch {
      log('WARN', `JSON invalide [${topic}]`);
      return;
    }
...
async function loadAuthorizedFaces() {
  ...
  for (const person of persons) {
    ...
    if (imgs.length === 0) {
      log('WARN', `⚠️ Aucune image pour ${person}`);
      continue;
    }
...
async function analyzeFaceFromBuffer(buffer, authorizedFaces) {
  if (!authorizedFaces || authorizedFaces.length === 0) return { status: 'error', message: 'Base de visages vide' };

  const img = await canvas.loadImage(buffer);
  const detection = await faceapi.detectSingleFace(img).withFaceLandmarks().withFaceDescriptor();
  if (!detection) return { status: 'no_face', message: 'Aucun visage détecté' };
...
app.post('/api/save-token', express.json(), (req, res) => {
  ...
  if (token && typeof token === 'string' && token.length > 0) {
    ...
    log('INFO', `✅ Token FCM reçu et stocké : ${token.substring(0, 20)}...`);
...
app.post('/access', async (req, res) => {
  ...
  const accessDoc = await Access.create({
    ...
    log('ERROR', `MongoDB accesses : ${err.message}`);
    return null;
  });
...
app.get('/status', (req, res) =>
  res.json({
    ...
    websocket: { enabled: true, url: `ws://0.0.0.0:${CONFIG.port}`, clients: wss ? wss.clients.size : 0 },
    ...
    fallback: { uids_en_fallback: fallbackState.size, fenetre_ms: CONFIG.fallbackWindowMs },
...
async function start() {
  ...
  process.on('unhandledRejection', (reason) => {
    log('ERROR', `[FATAL] Promise : ${reason}`);
    process.exit(1);
  });
}
