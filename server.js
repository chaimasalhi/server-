...
// ============================================================
//  IoT Backend — Salle Réseau v12.1 — optimisé Render
// ============================================================

// Réduire le nombre de threads pour économiser la RAM sur Render
...
// ─────────────────────────────────────────────
//  MONGODB — 8 COLLECTIONS (schémas inchangés)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  ÉTAT EN MÉMOIRE (inchangé)
// ─────────────────────────────────────────────
const tempBuffer = [];
const tempState  = { level: 'NORMAL' };
const humidityState = { level: 'NORMAL', debounceCount: 0, window: [] };
const gasState = { lastAlert: false, alertCooldown: 0, prevEma: null };
const rainState = { raining: null, rainStartTime: null, dryCount: 0, lastLevel: 'NORMAL' };
const liveData = { temperature: null, humidity: null, gas: null, rain: null, prediction: null, lastUpdate: null };
...
// ─────────────────────────────────────────────
//  HMAC, saveAlert, CSV, etc. (inchangés)
// ─────────────────────────────────────────────
function verifyHmac(payload) {
  ...
  const expectedHmac = crypto.createHmac('sha256', CONFIG.hmacKey).update(dataStr).digest('hex');
  try {
  ...
  try {
    ...
    wsBroadcast({
      ...
      alert: {
        ...
        sensor, niveau, source, message,
        valeur: valeur ?? null, seuil: seuil ?? null, unit: unit ?? null,
        minutes_avant: minutesAvant ?? null, confidence: confidence ?? null,
...
// ─────────────────────────────────────────────
//  TEMPÉRATURE, HUMIDITÉ, GAZ, PLUIE (inchangés)
// ─────────────────────────────────────────────
...
async function predictTemperature(temperature) {
  ...
  if (tempBuffer.length < 6) {
    const result = { temperatureActuelle: temperature, temperaturePrediteA20: null, minutesAvantWarning: null, minutesAvantDanger: null, alerte: 'NORMAL', confidence: 0, message: `⏳ Collecte en cours (${tempBuffer.length}/6)`, timestamp: new Date().toLocaleString() };
    liveData.prediction = result;
    return result;
  }
  const trend = tempBuffer[tempBuffer.length - 1] - tempBuffer[0];
  if (trend <= 0) {
    const result = { temperatureActuelle: temperature, temperaturePrediteA20: null, minutesAvantWarning: null, minutesAvantDanger: null, alerte: 'NORMAL', confidence: 0.95, message: '✅ Tendance stable ou décroissante', timestamp: new Date().toLocaleString() };
    liveData.prediction = result;
    return result;
  }
  ...
  let minutesAvantWarning = null, minutesAvantDanger = null, predictedAt20min = null;
  ...
  let alerte = 'NORMAL', message = '✅ Aucun dépassement prévu dans les 60 prochaines minutes';
  if (minutesAvantDanger !== null && confidence >= CONFIG.predConfidence) {
    ...
    if (cooldownOk) {
      await saveAlert('temperature', alerte, message, 'prediction', temperature, CONFIG.salleMax, minutesAvantDanger, confidence, '°C', 'ASHRAE TC 9.9 A1 §6.3');
    ...
    if (cooldownOk) {
      await saveAlert('temperature', 'WARNING', message, 'prediction', temperature, CONFIG.salleWarning, minutesAvantWarning, confidence, '°C', 'ASHRAE TC 9.9 A1 §6.3');
      lastPredAlertTs = nowMs;
    }
  }
  const result = { temperatureActuelle: temperature, temperaturePrediteA20: predictedAt20min, minutesAvantWarning, minutesAvantDanger, dansMinutes: CONFIG.predictAhead, confidence, alerte, message, timestamp: new Date().toLocaleString() };
  liveData.prediction = result;
  return result;
}
...
async function handleTemperature(payload) {
  ...
  saveToCSV(temperature, avgTemp, now.classification, predict.temperaturePrediteA20, predict.minutesAvantWarning, predict.minutesAvantDanger, predict.alerte, predict.confidence);
  await TempReading.create({ device_id: CONFIG.deviceId, location: CONFIG.location, classification: now.classification, temperature, avg_temp: avgTemp, predicted_temp: predict.temperaturePrediteA20, minutes_warning: predict.minutesAvantWarning, minutes_danger: predict.minutesAvantDanger, prediction_alert: predict.alerte, pred_confidence: predict.confidence, reason: now.reason }).catch(err => log('ERROR', `MongoDB temperatures : ${err.message}`));
  if (now.isAlert) {
    const seuil = now.classification === 'DANGER' ? CONFIG.dangerTemp : CONFIG.warningTemp;
    await saveAlert('temperature', now.classification, now.reason, 'mesure', temperature, seuil, null, null, '°C', 'ASHRAE TC 9.9 A1 §6.3');
  }
  liveData.temperature = { temperature, avg_temp: avgTemp, classification: now.classification, reason: now.reason, prediction: predict };
...
async function handleHumidity(payload) {
  ...
  const trend = humidityState.window.length >= 3 ? humidityState.window[humidityState.window.length - 1] - humidityState.window[0] : 0;
  let targetLevel = 'NORMAL', riskType = null;
  ...
  if (confirmed && wasNormal && riskType) {
    ...
    const seuil = riskType === 'condensation' ? CONFIG.humidityDanger : riskType === 'esd_severe' ? CONFIG.humidityDangerLow : riskType === 'warning_haut' ? CONFIG.humidityMax : CONFIG.humidityMin;
    await saveAlert('humidity', targetLevel, alertMsg, 'mesure', humidity, seuil, null, null, '%HR', 'ASHRAE TC 9.9 / TIA-942-B §6.5');
  }
  if (!confirmed && !wasNormal && humidityState.debounceCount === 0) {
    await saveAlert('humidity', 'INFO', `✅ Humidité revenue normale : ${humidity.toFixed(1)}%HR`, 'mesure', humidity, null, null, null, '%HR', 'ASHRAE TC 9.9 A1');
  }
  humidityState.level = classification;
  const icon = { NORMAL: '✅', WARNING: '⚠️', DANGER: '🚨' }[classification];
  log('INFO', `${icon} Humid: ${humidity.toFixed(1)}%HR | trend:${trend > 0 ? '+' : ''}${trend.toFixed(1)}% | debounce:${humidityState.debounceCount}/${CONFIG.humidityDebounce} → ${classification}`);
  await HumidReading.create({ device_id: CONFIG.deviceId, location: CONFIG.location, classification, humidity, avg_humidity: avgHumidity, humidity_trend: +trend.toFixed(2), debounce_count: humidityState.debounceCount }).catch(err => log('ERROR', `MongoDB humidites : ${err.message}`));
  liveData.humidity = { humidity, avg_humidity: avgHumidity, classification, trend: +trend.toFixed(2), debounce: humidityState.debounceCount, risk_type: riskType };
  liveData.lastUpdate = new Date().toLocaleString();
  wsBroadcast({ type: 'live_data', data: liveData });
}
...
async function handleGas(payload) {
  ...
  if (classification === 'DANGER' && !wasAlert) {
    await saveAlert('gas', 'DANGER', buildGasAlertMessage('DANGER', adcEma, adcRaw, risingRate), 'mesure', adcRaw, CONFIG.gasDanger, null, null, 'ADC', 'IEC 60079 / EN 54-5');
    gasState.lastAlert = true;
    gasState.alertCooldown = now + CONFIG.gasCooldown * 1000;
  } else if (classification === 'WARNING' && !wasAlert) {
    await saveAlert('gas', 'WARNING', buildGasAlertMessage('WARNING', adcEma, adcRaw, risingRate), 'mesure', adcRaw, CONFIG.gasThreshold, null, null, 'ADC', 'IEC 60079 / EN 54-5');
  } else if (classification === 'NORMAL' && wasAlert && !inCooldown) {
    await saveAlert('gas', 'INFO', `✅ Gaz revenue normale : EMA=${adcEma} ADC`, 'mesure', adcRaw, null, null, null, 'ADC', 'IEC 60079');
  ...
  await GazReading.create({ device_id: CONFIG.deviceId, location: CONFIG.location, classification, adc_raw: adcRaw, voltage, adc_ema: adcEma, gas_rising_rate: risingRate, alert_raw: alert }).catch(err => log('ERROR', `MongoDB gaz : ${err.message}`));
  ...
  if (raining) {
    ...
    if (rainState.raining) {
      ...
      if (rainState.dryCount >= CONFIG.rainDryConfirm) {
        ...
        classification = 'WARNING';
        rainDurationMin = rainState.rainStartTime ? +((now - rainState.rainStartTime) / 60000).toFixed(1) : null;
      }
    }
  }
  const prevLevel = rainState.lastLevel;
  if (raining && prevLevel === 'NORMAL') {
    await saveAlert('rain', 'WARNING', `⚠️ Fuite condensat détectée — TIA-942-B §6.6`, 'mesure', null, null, null, null, 'min', 'TIA-942-B §6.6');
  } else if (raining && rainDurationMin !== null && rainDurationMin >= CONFIG.rainDangMinutes && prevLevel !== 'DANGER') {
    await saveAlert('rain', 'DANGER', `🚨 DANGER fuite prolongée : ${rainDurationMin} min — TIA-942-B §6.6`, 'mesure', rainDurationMin, CONFIG.rainDangMinutes, null, null, 'min', 'TIA-942-B §6.6');
  } else if (!raining && rainState.dryCount === 0 && prevLevel !== 'NORMAL') {
    await saveAlert('rain', 'INFO', `✅ Fuite terminée — retour sec confirmé`, 'mesure', null, null, null, null, 'min', 'TIA-942-B §6.6');
  }
  rainState.lastLevel = classification;
  const icon = { NORMAL: '✅', WARNING: '⚠️', DANGER: '🚨' }[classification];
  log('INFO', `${icon} Eau: ${raining ? 'CONDENSAT' : 'sec'} | ${rainDurationMin ?? '—'}min → ${classification}`);
  await EauReading.create({ device_id: CONFIG.deviceId, location: CONFIG.location, classification, raining, rain_status: rainStatus, rain_duration_min: rainDurationMin, dry_count: rainState.dryCount }).catch(err => log('ERROR', `MongoDB eaux : ${err.message}`));
...
function connectMQTT() {
  ...
  client.on('message', async (topic, rawMsg) => {
    ...
    if (!verifyHmac(payload)) { log('WARN', `[SEC] Message rejeté [${topic}]`); return; }
    try {
...
// Compression des descripteurs : garder au maximum `maxDesc` par personne
function compressDescriptors(labeledFaces, maxDescriptorsPerPerson = 3) {
  return labeledFaces.map(labeled => {
    const descriptors = labeled.descriptors.slice(0, maxDescriptorsPerPerson);
    return new faceapi.LabeledFaceDescriptors(labeled.label, descriptors);
  });
}
...
async function loadEncodingsFromDB() {
  try {
    ...
    return compressDescriptors(labeled, 3); // ne garder que 3 descriptors max
  } catch (err) {
...
async function loadAuthorizedFaces() {
  ...
  return compressDescriptors(labeled, 3); // compression
}
...
async function initializeFaceEncodings() {
  ...
  const diskPersons = fs.existsSync(CONFIG.visagesDir)
    ? fs.readdirSync(CONFIG.visagesDir).filter(f => fs.lstatSync(path.join(CONFIG.visagesDir, f)).isDirectory())
    : [];
...
async function fetchCamPhoto() {
  ...
  try {
    const camResp = await fetch(CONFIG.camUrl, { method: 'GET', signal: controller.signal });
    clearTimeout(timer);
...
app.post('/analyze', upload.single('photo'), async (req, res) => {
  try {
    ...
    if (result.status === 'unauthorized') {
      ...
      await saveAlert('visage', 'DANGER', `🚨 Tentative accès — visage non reconnu (meilleur : ${result.best_attempt} à ${result.confidence})`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
    }
...
app.post('/access', async (req, res) => {
  ...
  const now = Date.now();
  if (!rfidOk) {
    await Access.create({ uid, authorized: false, datetime: datetime || new Date().toLocaleString(), rfid_status: 'refusé', face_result: 'non_analysé', decision: 'REFUSÉ_RFID', fallback: false, has_photo: false }).catch(err => log('ERROR', `MongoDB accesses : ${err.message}`));
    await saveAlert('acces', 'WARNING', `⚠️ Accès refusé — Badge UID inconnu : ${uid}`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
  ...
  if (fb && fb.usedFallback && (now - fb.timestamp) < CONFIG.fallbackWindowMs) {
    ...
    await Access.create({ uid, authorized: true, datetime: datetime || new Date().toLocaleString(), rfid_status: 'autorisé', face_result: 'non_analysé', decision: 'REFUSÉ_FALLBACK', fallback: true, has_photo: false }).catch(err => log('ERROR', `MongoDB accesses : ${err.message}`));
    await saveAlert('acces', 'DANGER', `🚨 Fallback déjà consommé — UID: ${uid}. Accès bloqué. Intervention requise.`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
    return res.json({ status: 'ok', decision: 'REFUSÉ_FALLBACK', uid, message: '❌ Accès unique secours déjà utilisé' });
  }
  let faceResult = 'non_analysé', faceName = null, faceConfidence = null, decision = 'AUTORISÉ', usedFallback = false, photoBuffer = null;
  const { success, camJson } = await fetchCamPhoto();
  if (!success) {
    usedFallback = true; decision = 'AUTORISÉ_FALLBACK'; faceResult = 'non_analysé';
    fallbackState.set(uid, { usedFallback: true, timestamp: now });
    await saveAlert('acces', 'WARNING', `⚠️ Accès FALLBACK — CAM hors ligne. UID: ${uid}. Accès unique consommé. Surveillance manuelle requise.`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
    log('WARN', `⚠️ FALLBACK accordé [${uid}]`);
  } else {
    const status = camJson?.status || 'error';
    faceConfidence = camJson?.confidence || null;
    if (status === 'authorized') {
      faceName = camJson.name || null; faceResult = 'reconnu'; decision = 'DOUBLE_AUTORISÉ'; fallbackState.delete(uid);
      log('INFO', `✅✅ DOUBLE_AUTORISÉ [${uid}] — ${faceName} (${faceConfidence})`);
    } else if (status === 'unauthorized') {
      faceResult = 'inconnu'; decision = 'REFUSÉ_VISAGE';
      await saveAlert('acces', 'DANGER', `🚨 BADGE OK mais VISAGE REFUSÉ — UID: ${uid}. Possible usurpation de badge !`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
      log('WARN', `🚨 REFUSÉ_VISAGE [${uid}]`);
    } else if (status === 'no_face') {
      faceResult = 'aucun_visage'; decision = 'REFUSÉ_VISAGE';
      await saveAlert('acces', 'WARNING', `⚠️ Aucun visage détecté — badge autorisé UID: ${uid}. Accès refusé.`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
      log('WARN', `⚠️ Aucun visage [${uid}]`);
    } else {
      usedFallback = true; decision = 'AUTORISÉ_FALLBACK'; faceResult = 'erreur';
      fallbackState.set(uid, { usedFallback: true, timestamp: now });
      await saveAlert('acces', 'WARNING', `⚠️ Erreur analyse IA — FALLBACK accordé pour UID: ${uid}. Accès unique consommé.`, 'mesure', null, null, null, null, null, 'TIA-942-B §5.3');
  ...
  const accessDoc = await Access.create({ uid, authorized: true, datetime: datetime || new Date().toLocaleString(), rfid_status: 'autorisé', face_result: faceResult, face_name: faceName, face_confidence: faceConfidence, decision, fallback: usedFallback, has_photo: photoBuffer !== null }).catch(err => { log('ERROR', `MongoDB accesses : ${err.message}`); return null; });
  if (photoBuffer && accessDoc) {
    try {
      await AccessPhoto.create({ access_id: accessDoc._id, uid, face_name: faceName, decision, photo_buffer: photoBuffer, photo_size: photoBuffer.length, mime_type: 'image/jpeg' });
      log('INFO', `📷 [DB] Photo stockée — access_id: ${accessDoc._id} | ${photoBuffer.length} octets`);
    } catch (err) {
      log('ERROR', `Stockage photo MongoDB : ${err.message}`);
    }
  }
  const icons = { 'DOUBLE_AUTORISÉ': '✅✅', 'AUTORISÉ': '✅', 'AUTORISÉ_FALLBACK': '⚠️', 'REFUSÉ_VISAGE': '❌', 'REFUSÉ_RFID': '❌', 'REFUSÉ_FALLBACK': '⛔' };
  ...
  res.json({ status: 'ok', decision, name: faceName, uid, fallback: usedFallback, message: accessGranted ? `✅ Accès accordé — ${decision}` : '❌ Accès refusé', timestamp: new Date().toISOString() });
});

app.get('/donnees', (req, res) => res.json(liveData));
app.get('/prediction', (req, res) => res.json(liveData.prediction || { message: 'En attente...' }));
app.get('/status', (req, res) => res.json({ status: 'running', version: 'v12.1', uptime: Math.round(process.uptime()) + 's', device: CONFIG.deviceId, location: CONFIG.location, topics: TOPICS, bufferSize: tempBuffer.length, websocket: { enabled: true, url: `ws://0.0.0.0:${CONFIG.port}`, clients: wss ? wss.clients.size : 0 }, collections: ['temperatures', 'humidites', 'gaz', 'eaux', 'alerts', 'accesses', 'face_encodings', 'access_photos'], normes: { temperature: 'ASHRAE TC 9.9 Classe A1 §6.3', humidite: 'ASHRAE TC 9.9 / ANSI/TIA-942-B §6.5', gaz: 'IEC 60079 / EN 54-5', condensat: 'TIA-942-B §6.6', acces: 'TIA-942-B §5.3' }, seuils: { temperature: { normal: `< ${CONFIG.warningTemp}°C`, warning: `${CONFIG.warningTemp}–${CONFIG.dangerTemp}°C`, danger: `≥ ${CONFIG.dangerTemp}°C` }, humidite: { normal: `${CONFIG.humidityMin}–${CONFIG.humidityMax}%HR`, danger_haut: `> ${CONFIG.humidityDanger}%HR`, danger_bas: `< ${CONFIG.humidityDangerLow}%HR` }, gaz: { normal: `< ${CONFIG.gasThreshold} ADC`, warning: `${CONFIG.gasThreshold}–${CONFIG.gasDanger} ADC`, danger: `≥ ${CONFIG.gasDanger} ADC` }, condensat: { warning: 'Dès la 1ère détection', danger: `≥ ${CONFIG.rainDangMinutes} min` } }, reconnaissance_faciale: { personnes_autorisees: app.locals.authorizedFaces?.length ?? 0, seuil_distance: CONFIG.faceThreshold, confiance_min: `${CONFIG.faceConfidenceMin * 100}%`, dossier: CONFIG.visagesDir, intrusions_dir: CONFIG.intrusionsDir }, fallback: { uids_en_fallback: fallbackState.size, fenetre_ms: CONFIG.fallbackWindowMs }, security: { hmacEnabled: true, apiKeyRequired: '/db/* et /logs', tlsMqtt: true }, states: { temperature: { level: tempState.level }, humidity: { level: humidityState.level, debounce: humidityState.debounceCount }, gas: { lastAlert: gasState.lastAlert, cooldownMs: Math.max(0, gasState.alertCooldown - Date.now()) }, rain: { raining: rainState.raining, dryCount: rainState.dryCount } }, liveData }));

app.get('/logs', requireApiKey, async (req, res) => { try { const logs = await Access.find().sort({ timestamp: -1 }).limit(20); res.json({ total: await Access.countDocuments(), logs }); } catch (e) { res.status(500).json({ error: e.message }); } });
...
app.get('/db/photos', requireApiKey, async (_, res) => { try { const photos = await AccessPhoto.find({}, { photo_buffer: 0 }).sort({ timestamp: -1 }).limit(50); res.json(photos); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/db/photos/:access_id', requireApiKey, async (req, res) => { try { const photo = await AccessPhoto.findOne({ access_id: req.params.access_id }); if (!photo) return res.status(404).json({ error: 'Photo non trouvée' }); res.set('Content-Type', photo.mime_type); res.set('Content-Disposition', `inline; filename="access_${photo.access_id}.jpg"`); res.send(photo.photo_buffer); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/db/photos/uid/:uid', requireApiKey, async (req, res) => { try { const photos = await AccessPhoto.find({ uid: req.params.uid }, { photo_buffer: 0 }).sort({ timestamp: -1 }).limit(20); res.json(photos); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/db/encodings', requireApiKey, async (_, res) => { try { const encs = await FaceEncoding.find({}, { descriptors: 0 }); res.json(encs); } catch (e) { res.status(500).json({ error: e.message }); } });
app.post('/faces/reload', requireApiKey, async (_, res) => { try { log('INFO', '🔄 Rechargement forcé encodages depuis disque...'); const labeled = await loadAuthorizedFaces(); if (labeled.length > 0) { await saveEncodingsToDB(labeled); app.locals.authorizedFaces = labeled; } res.json({ status: 'ok', message: `✅ ${labeled.length} personne(s) rechargée(s) et sauvegardées en DB`, persons: labeled.map(l => ({ name: l.label, descriptors: l.descriptors.length })) }); } catch (e) { log('ERROR', `/faces/reload : ${e.message}`); res.status(500).json({ error: e.message }); } });

// API publiques Flutter
app.get('/api/alertes', async (req, res) => { try { const limite = parseInt(req.query.limite) || 50; const type = req.query.type || null; const resolue = req.query.resolue !== undefined ? req.query.resolue === 'true' : null; const filter = {}; if (type) filter.sensor = type; if (resolue !== null) filter.resolue = resolue; const alertes = await Alert.find(filter).sort({ timestamp: -1 }).limit(limite).lean(); const normalized = alertes.map(a => ({ _id: a._id.toString(), type: a.sensor, niveau: a.niveau, source: a.source, message: a.message, valeur: a.valeur ?? null, seuil: a.seuil ?? null, unit: a.unit ?? null, horodatage: a.timestamp, resolue: a.resolue ?? false, minutes_avant: a.minutes_avant ?? null, confidence: a.confidence ?? null })); res.json({ alertes: normalized, total: normalized.length }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/alertes/stats', async (req, res) => { try { const now = new Date(); const il24h = new Date(now - 24 * 60 * 60 * 1000); const [total, nonResolues, critiques, dernieres24h] = await Promise.all([Alert.countDocuments(), Alert.countDocuments({ resolue: { $ne: true }, niveau: { $in: ['DANGER', 'WARNING'] } }), Alert.countDocuments({ niveau: 'DANGER', resolue: { $ne: true } }), Alert.countDocuments({ timestamp: { $gte: il24h } })]); res.json({ total, nonResolues, critiques, dernieres24h }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/alertes/:id/resoudre', async (req, res) => { try { const alerte = await Alert.findByIdAndUpdate(req.params.id, { resolue: true, resolue_at: new Date() }, { new: true }); if (!alerte) return res.status(404).json({ error: 'Alerte non trouvée' }); res.json({ success: true, alerte }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/journaux', async (req, res) => { try { const limite = parseInt(req.query.limite) || 50; const statut = req.query.statut || null; const filter = {}; if (statut === 'autorise') filter.authorized = true; if (statut === 'refuse') filter.authorized = false; const journaux = await Access.find(filter).sort({ timestamp: -1 }).limit(limite).lean(); const normalized = journaux.map(j => ({ _id: j._id.toString(), uid: j.uid, prenom: j.face_name?.split(' ')[0] ?? 'Inconnu', nom: j.face_name?.split(' ').slice(1).join(' ') ?? '', statut: j.authorized ? 'autorise' : 'refuse', typeAcces: j.decision ?? 'RFID', decision: j.decision, fallback: j.fallback, has_photo: j.has_photo, horodatage: j.timestamp })); res.json({ journaux: normalized, total: normalized.length }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/journaux/stats', async (req, res) => { try { const now = new Date(); const il24h = new Date(now - 24 * 60 * 60 * 1000); const [total, autorises, refuses, dernieres24h] = await Promise.all([Access.countDocuments(), Access.countDocuments({ authorized: true }), Access.countDocuments({ authorized: false }), Access.countDocuments({ timestamp: { $gte: il24h } })]); res.json({ total, autorises, refuse: refuses, dernieres24h }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/rfid', async (req, res) => { try { const rfids = await Access.find({ authorized: true }).distinct('uid'); res.json({ rfids: rfids.map(uid => ({ uid, actif: true })) }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/historique', async (req, res) => { try { const limite = parseInt(req.query.limite) || 200; const data = await TempReading.find().sort({ timestamp: -1 }).limit(limite).lean(); res.json({ historique: data, total: data.length }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/historique/stats', async (req, res) => { try { const [total, danger, warning] = await Promise.all([TempReading.countDocuments(), TempReading.countDocuments({ classification: 'DANGER' }), TempReading.countDocuments({ classification: 'WARNING' })]); res.json({ total, danger, warning, normal: total - danger - warning }); } catch (err) { res.status(500).json({ error: err.message }); } });

app.get("/api/visages", auth, isAdmin, (req, res) => { try { const visagesDir = CONFIG.visagesDir; if (!fs.existsSync(visagesDir)) return res.json({ visages: [] }); const dossiers = fs.readdirSync(visagesDir, { withFileTypes: true }).filter(entry => entry.isDirectory()).map(entry => entry.name); const visages = []; for (const dossier of dossiers) { const dirPath = path.join(visagesDir, dossier); const fichiers = fs.readdirSync(dirPath).filter(f => /\.(jpg|jpeg|png)$/i.test(f)); if (fichiers.length > 0) { visages.push({ nom: dossier, fichier: fichiers[0], count: fichiers.length, chemin: `/api/visages/photo/${dossier}/${fichiers[0]}` }); } } res.json({ total: visages.length, visages }); } catch (err) { console.error("❌ GET /api/visages ERROR:", err); res.status(500).json({ message: "Erreur serveur" }); } });
app.get("/api/visages/photo/:person/:filename", auth, isAdmin, (req, res) => { const { person, filename } = req.params; const filePath = path.join(CONFIG.visagesDir, person, filename); if (!fs.existsSync(filePath)) return res.status(404).json({ message: "Photo introuvable" }); res.sendFile(filePath); });

app.use((req, res) => res.status(404).json({ error: `Route inconnue : ${req.method} ${req.path}` }));

setInterval(() => { const cooldownS = Math.max(0, Math.ceil((gasState.alertCooldown - Date.now()) / 1000)); log('INFO', '── Diagnostics v12.1 ────────────────────────────────────────────'); log('INFO', `  [temp     ] ${liveData.temperature?.temperature ?? 'N/A'}°C → ${tempState.level}`); log('INFO', `  [pred     ] +20min=${liveData.prediction?.temperaturePrediteA20 ?? '?'}°C | conf=${liveData.prediction?.confidence ?? 'N/A'} → ${liveData.prediction?.alerte ?? 'N/A'}`); log('INFO', `  [humid    ] ${liveData.humidity?.humidity ?? 'N/A'}%HR → ${humidityState.level} | debounce:${humidityState.debounceCount}`); log('INFO', `  [gaz      ] ADC=${liveData.gas?.adc_raw ?? 'N/A'} | EMA=${liveData.gas?.adc_ema ?? 'N/A'} | cooldown:${cooldownS}s → ${liveData.gas?.classification ?? 'N/A'}`); log('INFO', `  [condensat] ${liveData.rain?.raining ? 'FUITE' : 'sec'} | ${liveData.rain?.duration_min ?? '—'}min → ${liveData.rain?.classification ?? 'N/A'}`); log('INFO', `  [visages  ] ${app.locals.authorizedFaces?.length ?? 0} personne(s) | seuil=${CONFIG.faceThreshold}`); log('INFO', `  [fallback ] ${fallbackState.size} UID(s) en mode secours`); log('INFO', `  [websocket] ${wss ? wss.clients.size : 0} client(s) connecté(s)`); log('INFO', '─────────────────────────────────────────────────────────────────'); }, 60000);
...
async function start() {
  ...
  const disableFace = process.env.DISABLE_FACE === 'true';
  if (!disableFace) {
    await loadFaceModels();
    app.locals.authorizedFaces = await initializeFaceEncodings();
  } else {
    log('INFO', '🔵 Reconnaissance faciale désactivée (DISABLE_FACE=true)');
    app.locals.authorizedFaces = [];
  }
